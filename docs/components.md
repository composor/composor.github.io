...
// Then update them with data.
const fruitsList = new List()
fruitsList.update(fruits)

const petsList = new List()
petsList.update(pets)

const toolsList = new List()
toolsList.update(tools)
```
...
You can use whatever server-side solution to pre-render the html for your document. Then after page loads, you can let Composi take over parts of the document as components. To do this you need to follow a simple rule:
    ...
    Give your component's main element a unique id that matches the id of an element in the rendered document. This needs to be in the same element as the component's container. Let's take a look at how we might do this. Suppose on the server we output some markup as follows:
